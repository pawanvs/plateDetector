# Number Plate Detector API — PoC Requirements

> **Document**: requirements.md  
> **Scope**: 1-week PoC (student project)  
> **Goal**: Dockerized API that detects a vehicle number plate and extracts text (and state where possible).
...
This is a **1-week PoC** for college students. Accuracy is expected to be good on the provided sample set, not perfect in every real-world scenario.
...
### 5.1 Success Response Requirements

- Must return `status: "ok"`
- Must return `confidence` as integer `0–100`
- Must return non-empty `result.plate_text`
...
### 5.2 Failure Response Requirements
...
- return `status: "fail"`
- use `error.code: "UNCLEAR"` or `"LOW_CONFIDENCE"`
...
1. `docker compose up --build` starts service successfully on Ubuntu.
2. `GET /health` returns `{ "status": "ok" }`.
3. `POST /v1/plate/detect` (upload) returns valid JSON in both success and fail cases.
4. Response always includes `status` and `meta.request_id`.
5. Failure responses always include `error.code` from the defined list.
6. Test script runs on the provided `samples/` and prints outputs without manual steps.
